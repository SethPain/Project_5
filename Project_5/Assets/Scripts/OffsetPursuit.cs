...
public class OffsetPursuit : MonoBehaviour {
    ...
    Vector3 targetPrevPos;          // The target's position on the last update
    Vector3 seekPosition;           // The position which the seeking object moves towards
    Vector3 velocity;               // This is the seeking object's velocity
    Vector3 targetVel;              // The target's velocity 
    ...
    public float LookAheadTime;     // Projected time at which the target follows a linear path

    void Awake()
    {
        target = GameObject.Find("Target"); // Find the target in the scene
        ...
        targetPosition = target.transform.position; // Initialize the target's position
        targetPrevPos = target.transform.position;  // Initialize the target's previous position
	...
	void Update () {
        ...
        targetVel = targetPosition - targetPrevPos; // Update target's velocity
        velocity = currentPosition - previousPosition;  // Calculate the seeking object's current velocity
        LookAheadTime = (targetPosition - currentPosition).magnitude / (targetVel.normalized + velocity.normalized).magnitude;  // Calculate LookAheadTime
        seekPosition = targetPosition + targetVel * LookAheadTime;  // Time the position which the seeker is moving towards
        Vector3 posDiff = seekPosition - currentPosition;   // Find the difference in disred position and current position
        if ((posDiff.x == 0) && (posDiff.z < 0))  // Find which side of the target the seekin object should be offset from
            seekPosition = seekPosition + new Vector3(0, 0, 3);
        else if ((posDiff.x < 0) && (posDiff.z < 0))
            seekPosition = seekPosition + new Vector3(3, 0, 3);
        else if ((posDiff.x < 0) && (posDiff.z == 0))
            seekPosition = seekPosition + new Vector3(3, 0, 0);
        else if ((posDiff.x < 0) && (posDiff.z > 0))
            seekPosition = seekPosition + new Vector3(3, 0, -3);
        else if ((posDiff.x == 0) && (posDiff.z > 0))
            seekPosition = seekPosition + new Vector3(0, 0, -3);
        ...
        desiredVel = (seekPosition - currentPosition).normalized * Speed * Time.deltaTime;    // Calculate the desired velocity
        ...
        targetPrevPos = target.transform.position; // Update target's previous position
